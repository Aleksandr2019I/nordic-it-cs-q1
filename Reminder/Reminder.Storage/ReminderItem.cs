...
namespace Reminder.Storage
{
	...
	{			
			...
			ReminderItemStatus status = ReminderItemStatus.Created)
		{
			if (id == default)
			{
				throw new ArgumentException("Индефикатор пустой",nameof(id));
			}

			if (string.IsNullOrWhiteSpace(contactId))
			{
				throw new ArgumentException("Индефикатор контакта пустой", nameof(contactId));
			}
		...
		public void ReadyToSend(ReminderItemStatus ready)
		{
			if (Status != ReminderItemStatus.Created)
			{
				throw new InvalidOperationException("некорретный статус");
			}
